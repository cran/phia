...
Now, \code{testInteractions} allows to define such custom
...
The functions of the package \pkg{phia} only report the values of the
...
from \pkg{car}, that contains several variables related to
...
confuse the functions of \pkg{phia}, so we will do this
