...
On the other hand, the flexibility of \R{} does allow to analyze
...
of a change in the level of one factor depends on the level of another
...
he used for demonstrating how to analyze interaction contrasts in linear models,
...
way, and the types of models that can be analyzed is different (see
sections \ref{sec:repeated-measures} and \ref{sec:covariates}). Those functions
are specially devised to analyze and plot the interactions of highest
...
column of \code{Values}, we can obtain the following conclusions:
\begin{enumerate}
...
The main effect of \code{hour} could be analyzed similarly. As told
...
However, we have seen that the effects of factors may also be analyzed
...
which we can explore with \code{interactionMeans} (see figure \ref{fig:interaction-covariates})
and test with \code{testInteractions}, by just giving the name of the
...
and \code{type}, we could have analyzed it independently, since
...
can be analyzed by the methods explained in previous sections, although
...
distribution.%
} These models may be analyzed in \R{} with the functions included in the packages
...
The functions of package \pkg{phia} for the post-hoc analysis of mixed-effects
...
In addition, we will explore the strategy to analyze repeated-measures with a
...
quadratic components of IQ around $0$ (remember that the \code{IQ} variable is centered),
with the same sign as the IQ value \cite[p.~122]{Snijders1999}.
...
we want to analyze with more flexibility. For that, we first have to transform the
...
\cite{R-help:094765}.%
\footnote{Mixed models fitted with \code{lme} from package \code{nlme} do
...
and then use standard tests \cite{Faraway2006}. The function \code{Anova} of
package \pkg{car} produces tables with \emph{p}\nobreakdash-values based on
Wald tests, following the latter assumption \cite{R-sig-ME:014095}, and by
extension, the same happens with the test functions of package \pkg{phia},
